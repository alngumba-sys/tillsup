...
serve(async (req) => {
  ...
  try {
    const { phoneNumber, amount, businessId, branchId, saleId, description } = await req.json()

    if (!phoneNumber || !amount || !businessId) {
      throw new Error('phoneNumber, amount, and businessId are required')
    }
    ...
    if (!/^254[17]\d{8}$/.test(formattedPhone)) {
      throw new Error('Invalid Kenyan phone number format. Use 07XXXXXXXX or 254XXXXXXXXX')
    }
    ...
    const stkPushPayload = {
      ...
      AccountReference: saleId || `TILLSUP-${Date.now()}`,
      TransactionDesc: description || 'Tillsup Payment',
    }
    ...
    const { error: dbError } = await supabase
      ...
      .insert({
        ...
        status: 'pending',
        description: description || 'Tillsup Payment',
      })
