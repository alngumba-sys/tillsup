...
import { Search, UserPlus, Edit, Trash2, Copy, CheckCircle2, Building2, KeyRound, AlertCircle, Shield, DollarSign } from "lucide-react";
...
export function StaffManagementTab() {
  const { user, business, getStaffMembers, createStaff, updateStaff, deleteStaff, resetStaffPassword } = useAuth();
  ...
  const handleAddStaff = async () => {
    ...
    try {
      const result = await createStaff(
        ...
        formData.lastName,
        formData.role,
        formData.branchId // Pass branchId to createStaff
      );

      if (result.success && result.credentials) {
        // ═══════════════════════════════════════════════════════════════════
        // UPDATE STAFF WITH SALARY DATA (if enabled)
        // This is done after creation since createStaff returns the credentials
        // ═══════════════════════════════════════════════════════════════════
        if (formData.salaryEnabled) {
          // Find the newly created staff member - Refresh List
          const updatedStaffMembers = await getStaffMembers();
          setStaffMembers(updatedStaffMembers);
          
          const newStaff = updatedStaffMembers.find(s => s.email === formData.email);
